...
"""
Tests for the ellipse_overlap_grid module.
"""
...
def test_ellipse_overlap_grid(grid_size, maj_size, min_size, angle, use_exact,
                              subsample):
    """
    Test normalization of the overlap grid to make sure that a fully
    enclosed pixel has a value of 1.0.
    """
    g = ellipse_overlap_grid(-1.0, 1.0, -1.0, 1.0, grid_size, grid_size,
                             maj_size, min_size, angle, use_exact, subsample)
    assert_allclose(g.max(), 1.0)


@pytest.mark.parametrize('use_exact', use_exacts)
def test_ellipse_overlap_grid_no_readonly_inputs(use_exact):
    """
    ...
    """
    g = ellipse_overlap_grid(-5.0, 5.0, -5.0, 5.0, 10, 12, 4.0, 2.0, 0.5,
                             use_exact, 5)
