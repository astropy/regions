...
"""
Tests for the circle_overlap_grid module.
"""
...
def test_circle_overlap_grid(grid_size, circ_size, use_exact, subsample):
    """
    Test normalization of the overlap grid to make sure that a fully
    enclosed pixel has a value of 1.0.
    """
    g = circle_overlap_grid(-1.0, 1.0, -1.0, 1.0, grid_size, grid_size,
                            circ_size, use_exact, subsample)
    assert_allclose(g.max(), 1.0)


@pytest.mark.parametrize('use_exact', use_exacts)
def test_circle_overlap_grid_no_readonly_inputs(use_exact):
    """
    ...
    """
    g = circle_overlap_grid(-5.0, 5.0, -5.0, 5.0, 10, 12, 3.0, use_exact, 5)
